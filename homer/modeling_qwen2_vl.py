...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    #! The size of tensor a (2047) must match the size of tensor b (1037) at non-singleton dimension 2
...
class VisionFlashAttention2(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor, rotary_pos_emb: torch.Tensor = None, output_attentions: bool = False,
    ) -> torch.Tensor:
        seq_length = hidden_states.shape[0]
        q, k, v = self.qkv(hidden_states).reshape(seq_length, 3, self.num_heads, -1).permute(1, 0, 2, 3).unbind(0)
        q = apply_rotary_pos_emb_vision(q.unsqueeze(0), rotary_pos_emb).squeeze(0)
        ...
        if output_attentions:
            ...
            tmp_key_states = k  # [bsz, seq_len, num_heads, head_dim]
...
class Qwen2VLDecoderLayer(nn.Module):
    ...
    def _apply_pruning(self, hidden_states, metric):
        ...
        def _merge_power_max_last_calibrated(
            ...
            last_token_position = position_ids[..., -1][-1].item() # 3, 1, n_tokens
            position_ids -= last_token_position
            position_ids *= -1
            position_ids = position_ids.to(torch.int).to(device)

            # TODO: Load bias to device
            #! Bias removed since it requires 'calibrate before use' understanding
        ...
        if reducer is None:
            raise ValueError(
                f"Unknown reduction mode: {self.merge_manager.reduction_mode}"
            )
        ...
        if num_tokens_to_reduce > 0:
            ...
            #! The shape of the mask [1371] at index 0 does not match the shape of the indexed tensor [3, 1, 1371] at index 1
            ...
            self.merge_manager.set_layer_reduction_results(
                ...
                significance_weights=torch.ones(
                    (batch_size, seq_len), dtype=metric.dtype
                ),
            )
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, homer_args=None):
        super().__init__(config)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                pixel_values_videos = pixel_values_videos.type(self.visual.blocks[0].mlp.fc2.weight.dtype())
        ...
        if homer_prefix is not None:
            ...
            position_ids = position_ids + homer_prefix["last_position_id"] + 1
            past_key_values = homer_prefix["cache"]
            use_cache = True
    ...
    def __process_leaf(
        ...
        image_embeds = chunk.image_embeds[0]
        assert input_ids is not None, "input_ids must be provided"
        assert image_embeds is not None, "image_embeds must be provided"
    ...
    def __process_nonleaf(
        ...
        merged_chunk = Chunk.merge(l_chunk, r_chunk) # merge position_ids, attention_mask, reduction_mask, hidden_states
    ...
    def _merge(
        ...
        if not_reduce_left:
            ...
            ) # 8 -> 4 -> 2 -> 1 -> __process_leaf() -> Chunk instance
            r_chunk = self._merge(
                chunks=right_chunks,
                height=height - 1,
                target_len=r_target_len,
            ) # 8 -> 4 -> 2 -> 1 -> __process_leaf()
    ...
    def create_homer_prefix(self, 
        ...
        image_embeds = rearrange(image_embeds, "(t h w) c -> t h w c", t=t, h=h, w=w) # t x h x w x c
        split_dims = [4, 2, 2]
        assert torch.tensor(split_dims).prod() == num_chunks
        t_chunk_len = [t // split_dims[0]] * split_dims[0]
        h_chunk_len = [h // split_dims[1]] * split_dims[1]
        w_chunk_len = [w // split_dims[2]] * split_dims[2]
        #######################################################
        ...
        chunks = Chunk.make_chunks(
            ...
            h_chunk_len=[0]+h_chunk_len,
            w_chunk_len=[0]+w_chunk_len,
            t_chunk_len=[0]+t_chunk_len,
