...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def get_dtype(self):
        return self.blocks[0].attn.qkv.weight.dtype
...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    q_embed = (q * cos) + (rotate_half(q) * sin) #! cos, sin PE should be modulated # chunk.prune 에서 참조할 것
...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        query_states = query_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2) # bsz, n head, q len, head dim
        key_states = key_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
        ...
        """
        RIGHT
        qkv: torch.Size([5, 28, 1413, 128]) torch.Size([5, 4, 1413, 128]) torch.Size([5, 4, 1413, 128])
        -> Apply multi modal rotary embedding qkv: torch.Size([5, 28, 1413, 128]) torch.Size([5, 4, 1413, 128]) 
        WRONG
        1st
        qkv: torch.Size([1, 28, 1, 128]) torch.Size([1, 4, 1, 128]) torch.Size([1, 4, 1, 128])
        -> Apply multi modal rotary embedding
        qkv: torch.Size([1, 28, 4466, 128]) torch.Size([1, 4, 4466, 128])
        
        2nd
        torch.Size([1, 4466, 28, 128]) torch.Size([1, 8931, 28, 128]) torch.Size([1, 4466, 28, 128])
        cos.shape: 3, 1, 4466, 128 | sin.shape: 3, 1, 4466, 128
        """
        ...
        """
        wrong version
        query_states: 1, 28, 4466, 128
        key_states: 1, 4, 4466, 128 -> 1, 4, 8931, 128
        value_states: 1, 4, 1, 128 -> 1, 4, 4466, 128
        """
        # repeat k/v heads if n_kv_heads < n_heads
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        ...
        query_states = query_states.transpose(1, 2) # # bsz, q len, n head, head dim
        ...
        #! CORRECT VERSION FROM HOMER
        if output_attentions:
            attns = []
            for (q_single, k_single) in zip(query_states, key_states):
                tmp_query_states = q_single.unsqueeze(0)  # [bsz, seq_len, num_heads, head_dim]
                tmp_key_states = k_single.unsqueeze(0)  # [bsz, seq_len, num_heads, head_dim]
            ...
            attn_weights = torch.cat(attns, dim=0)
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    ##################### MODIFIED FROM ORIGINAL CODE #####################
    def _apply_pruning(self, hidden_states, metric, reduction_mode, v_mask, prune_ratio):
        
        def _merge_power_max_last_calibrated(
            num_tokens_to_reduce, reduction_mask, metric, **kwargs
        ):
            """
            reduction_mask: [torch.Tensor] Boolean mask to reduce the corresponding token index or not # bsz, seq_len
            metric: [torch.Tensor] QK^T attention score with the last token of query and whole key tokens | bsz, n_head, 1, seq_len
            """
            significance_weights = metric[:, :, -1, :].max(dim=1).values # bsz, seq_len
        ...
        def _q_text_k_image(metric, v_mask, ratios):
            """
            metric: bsz, n_head, seq_len, seq_len
            v_mask: visual token mask | bsz, seq_len
            Compute the average attention-score one token received from text tokens as the criteria attn in our experiment.
            """
            device = metric.device
            averaged_metric = torch.mean(metric, axis=1).float() # bsz, seq_len, seq_len
            if v_mask is not None: 
                B = v_mask.size(0)
            keep_masks = []
            sig_weights = []
            for b in range(B-1): #! exclude the last batch since this is the query
                original_metric = averaged_metric[b]  # shape: (N, N)
                v = v_mask[b]  # shape: (seq_len,) boolean mask
                submat = original_metric[~v, :][:, v] # shape (n_text, n_vis)
                col_indices = (v).nonzero(as_tuple=True)[0] # shape: (sum(v),)
                average_attn = submat.mean(dim=0) # shape: (n_vis, ), average attention that a visual token gets from other text tokens
                q_vi = average_attn.quantile(float(ratios[b]))
                ratio_mask = (average_attn < q_vi) # n_vis
                vis_token_mask = torch.ones_like(v, dtype=torch.bool).scatter_(0, col_indices[ratio_mask.cpu()], False)
                pruned_metric = torch.ones(v.size(0), device=device) * average_attn.min()
                pruned_metric[col_indices[ratio_mask.cpu()]] = average_attn[ratio_mask]
                keep_masks.append(vis_token_mask)
                sig_weights.append(pruned_metric)
            original_metric = averaged_metric[b]  # shape: (N, N)
            v = v_mask[b]  # shape: (seq_len,) boolean mask
            average_attn = original_metric[~v, :].mean(0) # shape: (n_text, seq_len)
            keep_masks.append(torch.ones_like(v, dtype=torch.bool))
            sig_weights.append(average_attn)
            return torch.stack(keep_masks), torch.stack(sig_weights) # shape: (bsz, seq_len)
        
        def _q_all_k_image(metric, v_mask, ratios):
            """
            metric: bsz, n_head, seq_len, seq_len
            v_mask: visual token mask | bsz, seq_len
            [FastV]: We simply compute the average attention-score one token received from all other tokens as the criteria attn in our experiment.
            """
            device = metric.device
            averaged_metric = torch.mean(metric, axis=1).float() # bsz, seq_len, seq_len
            if v_mask is not None: 
            ...
            for b in range(B-1): #! exclude the last batch since this is the query
                ...
                submat = original_metric[~v, :][:, v] # shape (n_text, n_vis)
                col_indices = (v).nonzero(as_tuple=True)[0]   # shape: (sum(v),)
                average_attn = submat.mean(dim=0) # n_vis, average attention that a visual token gets from other text tokens
                q_vi = average_attn.quantile(float(ratios[b]))
                ratio_mask = (average_attn < q_vi) # n_vis
                vis_token_mask = torch.ones_like(v, dtype=torch.bool).scatter_(0, col_indices[ratio_mask.cpu()], False)
                average_attn = torch.full_like(average_attn, fill_value=-torch.inf).scatter_(0, col_indices.cuda()[ratio_mask], average_attn)
                prune_masks.append(vis_token_mask)
                sig_weights.append(average_attn)
            return torch.stack(prune_masks), torch.stack(sig_weights) # 4, seq_len
            
        def _rater_pruning(metric, v_mask, prune_ratios):
            device = metric.device
            averaged_metric = torch.mean(metric, axis=1) .float() # bsz, seq_len, seq_len
            if v_mask is not None:
                masked_metric =  averaged_metric[~v_mask][:, v_mask] # bsz, text_seq_len, visioseq_len_len
            text_tokens = (masked_metric.values).mean(dim=2) # bsz, text_seq_len
            rater_quantile = torch.quantile(text_tokens, q=0.8, dim=1)
            rater_idxs = text_tokens > rater_quantile
            signif = masked_metric[rater_idxs].mean(dim=1) # bsz, vis_seq_len
            visual_quantile = signif > torch.cat(prune_ratios, dim=0).unsqueeze(1)
            indices_to_remove = (visual_quantile).nonzero().flatten()
            return indices_to_remove, _
            

        def _random_pruning(metric, v_mask, prune_ratios):
            device = metric.device
            averaged_metric = torch.mean(metric, axis=1) .float() # bsz, seq_len, seq_len
            if v_mask is not None:
                masked_metric =  averaged_metric[~v_mask][:, v_mask] # bsz, text_seq_len, visioseq_len_len
            num_tokens_to_reduce = masked_metric.size(2) * prune_ratios
            indices_to_remove = torch.randint(0, masked_metric.size(1), (num_tokens_to_reduce,))
            return indices_to_remove, None
            
        # Update this dictionary to add new reduction modes
        reducers = {
            "power_max_last_calibrated": _merge_power_max_last_calibrated,
            "query_text_key_image": _q_text_k_image,
            "query_all_key_image": _q_all_k_image,
            "rater_pruning": _rater_pruning,
            "random": _random_pruning,
        }
        assert reduction_mode in reducers.keys(), "reduction_mode not supported"
        reducer = reducers[reduction_mode]
        keep_masks, significance_weights = reducer( # bsz, seq_len & bsz, seq_len
                    metric = metric, # bsz, n_head, seq_len, seq_len
                    v_mask = v_mask, # bsz, seq_len, indicates vision token or not
                    ratios = prune_ratio, # pruning ratio for each batch
                )
        return keep_masks
    ######################################################################
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def _forward_and_prune(
        ...
        reduction_mode,
        v_mask,
        pad_masks, # batched inputs have paddings -> used when removing th when flattening batch sample into single sequence
        past_key_value = None,
        prune_layer = 1,
    ):
        ...
        for i, i_layer in enumerate(self.model.layers):
            ...
            if i == prune_layer:
                ...
                keep_masks = i_layer._apply_pruning(
                    hidden_states=hidden_states, # bsz, seq_len, n_dim
                    metric=self_attn_weights, # bsz, n_heads, seq_len, seq_len
                    reduction_mode=reduction_mode, 
                    v_mask=v_mask,
                    prune_ratio=ratio,
                )
                
                new_hidden_states, masks = [], []
                bsz = hidden_states.size(0)
                for b in range(bsz):
                    hs = hidden_states[b]
                    keep_mask = keep_masks[b] # True for tokens to be kept
                    pad_mask = pad_masks[b] # True for padding tokens
                    mask = ~pad_mask & keep_mask
                    masks.append(mask)
                    hs = hs[mask.to(hs.device)] # seq_len, n_dim
                    new_hidden_states.append(hs)
                hidden_states = torch.cat(new_hidden_states, dim=0).unsqueeze(0) # 1, total_seq_len, n_dim
                masks = torch.stack(masks, dim=0) # bsz, seq_len
                t, h, w = position_ids # 3, bsz, seq_len
                for idx in range(0, t.size(0) - 1):
                    t[idx + 1] += t[idx][-1] + 1 - pad_masks[idx].nonzero().size(0)
                    h[idx + 1] += h[idx][-1] + 1 - pad_masks[idx].nonzero().size(0)
                    w[idx + 1] += w[idx][-1] + 1 - pad_masks[idx].nonzero().size(0)
                position_ids = torch.stack([t, h, w], dim=0) # 3, bsz, seq_len
                position_ids = position_ids[..., masks].unsqueeze(1)
                key_cache = past_key_value.key_cache
                value_cache = past_key_value.value_cache
                n_head = key_cache[0].size(1)
                n_dim = key_cache[0].size(-1)
                
                past_key_value.key_cache = [
                    k[masks.unsqueeze(1).repeat(1, n_head, 1), :].reshape(1, n_head, -1, n_dim) for k in key_cache # bsz, num_heads, seq_len, n_dim_per_head
                ] # n_layers x (bsz, num_heads, seq_len, n_dim) -> 1, num_heads, seq_len x bsz, n_dim
                past_key_value.value_cache = [
                    v[masks.unsqueeze(1).repeat(1, n_head, 1), :].reshape(1, n_head, -1, n_dim) for v in value_cache # bsz, num_heads, seq_len, n_dim_per_head
                ]
        ...
        logits = self.lm_head(hidden_states)
        logits = logits.float()
        last_position_id = position_ids[0, :, -1]
        return {
