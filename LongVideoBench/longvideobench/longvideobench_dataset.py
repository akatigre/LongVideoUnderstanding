...
def load_video(video_file, duration, target_fps=1, max_num_frames=None):
    import av
    import numpy as np
...
class LongVideoBenchDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        frames, frame_timestamps = load_video(os.path.join(self.data_path, "videos", di["video_path"]), di["duration"], target_fps=self.target_fps,) #max_num_frames=self.max_num_frames)
        ...
        return {"inputs": inputs, "correct_choice": chr(ord("A")+di.get("correct_choice", -1)), **di}
