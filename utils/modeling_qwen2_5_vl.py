...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        if (
            ...
            sliding_window = None
        #################### MODIFIED FROM ORIGINAL CODE #####################
        #! WRONG VERSION THAT PRODUCES Bsz x Seq Length x N head x N head
        # attn_weights = torch.matmul(
        #     query_states, key_states.transpose(2, 3)
        # ) / math.sqrt(self.head_dim) # query_states: last dimension should be head_dim, key_states: second last should be head_dim -> batch, q_len, n_head, n_head
        # # bsz, q_len, n_head, head_dim | bsz, k_len, n_head, head_dim => bsz, seq_len, n_head, n_head
        # if attention_mask is not None:  # no matter the length, we just slice it
        #     causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        #     attn_weights = attn_weights + causal_mask
        ...
        if output_attentions:
            tmp_query_states = query_states[:, -1:, :, :]  # [bsz, query_len, num_heads, head_dim]
            tmp_key_states = key_states  # [bsz, kv_len, num_heads, head_dim]

            tmp_query_states = tmp_query_states.permute(
                0, 2, 1, 3
            )  # [bsz, num_heads, query_len, head_dim]
            tmp_key_states = tmp_key_states.permute(
                0, 2, 3, 1
            )  # [bsz, num_heads, head_dim, kv_len]

            attn_weights = torch.matmul(tmp_query_states, tmp_key_states) / math.sqrt(
                self.head_dim
            )  # [bsz, num_heads, query_len, kv_len]
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    def _apply_pruning(self, hidden_states, metric):
        ...
        def _merge_power_max_last_calibrated(
            ...
            #* CORRECT VERSION - last query token: bsz, n_head, seq_len -> For i-th key token and last query token, get maximum similarity among multiheads
            #* WRONG VERSION - last head: bsz, seq_len, n_head -> For i-th query token and i-th key token, get maximum similarity among multiheads
            #* metric: bsz, q_len, n_head, head_dim X bsz, k_len, n_head, head_dim -> bsz, q_len = k_len, n_head, n_head 
